...
MAXTIME: constant(uint256) = 182 * 86400  # half years
MULTIPLIER: constant(uint256) = 10 ** 9

VOTE_WEIGHT_MULTIPLIER: constant(uint256) = 4 - 1 # 4x gives 300% boost at half years
...
@external
@nonreentrant('lock')
def create_lock(_addr: address, _value: uint256, _unlock_time: uint256):
    """
    ...
    assert unlock_time <= block.timestamp + MAXTIME, "Voting lock can be half years max"
...
@external
@nonreentrant('lock')
def increase_amount(_addr: address, _value: uint256):
    """
    ...
    self._deposit_for(_addr, _value, 0, _locked, INCREASE_LOCK_AMOUNT)


@external
@nonreentrant('lock')
def increase_unlock_time(_addr: address, _unlock_time: uint256):
    """
    ...
    self._deposit_for(_addr, 0, unlock_time, _locked, INCREASE_UNLOCK_TIME)
...
@external
@view
def balanceOfAtNow(addr: address) -> uint256:
    """
...
@external
@view
def totalSupplyAtNow() -> uint256:
    """
    @notice Calculate total voting power at some point in the past
    @return Total voting power at `_block`
    """
    _block: uint256 = block.number
    _epoch: uint256 = self.epoch
...
@external
@view
def totalTokenSupplyAt(_block: uint256) -> uint256:
    """
